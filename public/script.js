// public/script.js - VERSIÓN FINAL Y COMPLETA
...
function showJoinModal() {
    const joinHTML = `
        ...
        <div>
            ...
            <div id="ai-warning" class="text-center text-amber-400 text-xs mt-2 min-h-[16px]"></div>
        </div>
        <div class="flex flex-col sm:flex-row gap-4">
            <button id="join-ai-button" class="w-full bg-cyan-500 text-white font-bold py-3 px-6 rounded-lg hover:bg-cyan-600 transition-all duration-300 transform hover:scale-105 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">Jugar contra IA</button>
    ...
    const aiButton = document.getElementById('join-ai-button');
    const warningDiv = document.getElementById('ai-warning');
    const sizeRadios = document.querySelectorAll('input[name="boardSize"]');

    function checkAISupport() {
        const selectedSize = document.querySelector('input[name="boardSize"]:checked').value;
        if (selectedSize !== '3') {
            aiButton.disabled = true;
            warningDiv.textContent = 'IA (Difícil) solo disponible en 3x3';
        } else {
            aiButton.disabled = false;
            warningDiv.textContent = '';
        }
    }
    sizeRadios.forEach(radio => radio.addEventListener('change', checkAISupport));
    checkAISupport();
